...
\textbf{Theorem.} A graph $G$ is strongly connected if and only if it can be constructed using the following procedure: \begin{enumerate}
...
\item Connect $v \rightarrow w_1 \rightarrow \cdots \rightarrow w_k \rightarrow u$, and put $w_1 \, \cdots \, w_k$ into $S$.
\end{enumerate}
\end{enumerate}

This process is known as ``ear decomposition'', and the trail $v \rightarrow w_1 \rightarrow \cdots \rightarrow w_k \rightarrow u$ is called an ``ear''.
